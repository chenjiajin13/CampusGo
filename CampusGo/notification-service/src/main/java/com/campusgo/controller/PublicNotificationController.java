...
public class PublicNotificationController {
    ...
    public List<NotificationDTO> inboxUser(@PathVariable("userId") Long userId) {
        return service.inbox(NotificationTargetType.USER, userId).stream().map(NotificationMapper::toDTO).collect(Collectors.toList());
    }
    ...
    public NotificationDTO send(@RequestBody NotificationCreateRequest req) {
        return NotificationMapper.toDTO(service.send(req.getTargetType(), req.getTargetId(), req.getChannel(), req.getTitle(), req.getContent(), req.getData()));
    }
    ...
    public List<NotificationDTO> inboxMerchant(@PathVariable("merchantId") Long merchantId) {
        return service.inbox(NotificationTargetType.MERCHANT, merchantId).stream().map(NotificationMapper::toDTO).collect(Collectors.toList());
    }
}